...
const formatFileSize = (sizeInMB) => {
    // Vérifie si fileSize est un nombre avant de le formater
    return typeof sizeInMB === 'number' ? `${sizeInMB.toFixed(1)} Mo` : `${sizeInMB} Mo`;
};
...
const TagBadge = ({ children }) => (
    ...
    <span className="text-xs font-medium text-blue-300 bg-blue-900/40 px-2 py-0.5 rounded-full mr-2 last:mr-0 transition duration-150">
        {children}
    </span>
);
...
const DocumentCard = ({ document }) => {
    ...
    const handleView = (e) => {
        e.preventDefault();
        // Logic to navigate to view page 
        console.log(`Vue de: ${title}`);
    };

    const handleShare = (e) => {
        e.preventDefault();
        // Logic to copy link to clipboard
        console.log(`Partage de: ${title}`);
    };
        ...
        <div className="flex border h-auto w-full border-[#bdbdbd25] bg-[#1a1a1a] p-4 rounded-lg transition duration-300">
            ...
            <div className="flex-grow flex flex-col justify-between text-gray-300 min-w-0">
                ...
                <div className="flex flex-col relative" >
                    ...
                    {/* Titre (navigation) */}
                    <Link to={`/document/${id}`} className="hover:text-white transition-colors">
                        <h2 className="w-auto text-nowrap truncate text-xl font-bold leading-snug">{title}</h2>
                    </Link>
                    ...
                    <div className="mt-3 text-sm flex items-center flex-nowrap overflow-x-auto gap-2">
                        ...
                        <span className="text-gray-700">|</span>
                        <span className="text-gray-500 text-nowrap">{formatFileSize(fileSize)}</span>
                ...
                <div className="flex w-full justify-between items-end mt-4">
                    ...
                    <div className="flex justify-center w-auto items-center gap-3 flex-nowrap pb-1.5 pr-2">
                        ...
                        <button onClick={handleDownload} className="flex items-center bg-gray-200 hover:bg-white text-black font-semibold py-1.5 px-4 rounded-md flex-shrink-0 transition"> 
                            <Download className="w-4 h-4 mr-1"/> 
                            <span className="text-sm">Télécharger</span>
                        </button>
                        
                        {/* Bouton Voir (Secondaire: Gris foncé) */}
                        <Link to={`/document/${id}`} onClick={handleView} className="flex items-center bg-[#2d2d2d] hover:bg-[#3a3a3a] text-gray-300 font-semibold py-1.5 px-4 rounded-md flex-shrink-0 transition"> 
