            ...
            <header className="flex justify-center bg-[#111] text-[hsl(0_0%_80%)] text-[1rem] border-b border-b-[#bdbdbd25] py-4 w-full fixed top-0 z-100 backdrop-blur-md h-16">
                ...
                <div className={`flex flex-col items-center w-70 fixed top-0 right-0 h-dvh z-110 bg-[#111] pt-20 text-gray-300 md:hidden ${isMenuOpen? 'translate-x-0 transition-transform duration-300 ease-in-out': 'translate-x-full transition-transform duration-300 ease-in-out'}`}>
                    <ul className='flex w-full flex-col items-center gap-8 p-10'>
                        <li className={`w-full text-xl text-[#bbb] font-bold border-b-[#bdbdbd25] pb-3 border-b hover:text-gray-100 transition-colors duration-300 ease-in-out active:text-white ${activeTab === 'home' ? 'text-white' : ''}`} onClick={() => { closeMenu(); setActiveTab('home'); }}><Link to="/">Accueil</Link></li>
                        <li className={`w-full text-xl text-[#bbb] font-bold border-b-[#bdbdbd25] pb-3 border-b hover:text-gray-100 transition-colors duration-300 ease-in-out active:text-white ${activeTab === 'community' ? 'text-white' : ''}`} onClick={() => { closeMenu(); setActiveTab('community'); }}><Link to="/community">Communauté</Link></li>
                        <li className={`w-full text-xl text-[#bbb] font-bold border-b-[#bdbdbd25] pb-3 border-b hover:text-gray-100 transition-colors duration-300 ease-in-out active:text-white ${activeTab === 'resources' ? 'text-white' : ''}`} onClick={() => { closeMenu(); setActiveTab('resources'); }}><Link to="/resources">Resources</Link></li>
                        <li className={`w-full text-xl text-[#bbb] font-bold border-b-[#bdbdbd25] pb-3 border-b hover:text-gray-100 transition-colors duration-300 ease-in-out active:text-white ${activeTab === 'upload' ? 'text-white' : ''}`} onClick={() => { closeMenu(); setActiveTab('upload'); }}><Link to="/upload">Televerser</Link></li>
                        <li className={`w-full text-xl text-[#bbb] font-bold border-b-[#bdbdbd25] pb-3 border-b hover:text-gray-100 transition-colors duration-300 ease-in-out active:text-white ${activeTab === 'profile' ? 'text-white' : ''}`} onClick={() => { closeMenu(); setActiveTab('profile'); }}><Link to="/profile">Profile</Link></li>
                        <li className={`w-full text-xl text-[#bbb] font-bold text-center border border-[#cccccc25] rounded-[6px] py-2 hover:bg-[#bbbbbb15] transition-colors duration-300 ease-in-out active:bg-[#bbbbbb15] ${activeTab === 'login' ? 'bg-[#bbbbbb15]' : ''}`} onClick={() => { closeMenu(); setActiveTab('login'); }}><Link to="/login">Se connecter</Link></li>
                        <li className={`w-full text-xl font-bold text-center bg-[hsl(0_0%_90%)] text-black -translate-y-4 rounded-[6px] py-2 hover:bg-[hsl(0_0%_85%)] transition-colors duration-300 ease-in-out active:bg-[hsl(0_0%_85%)] ${activeTab === 'signup' ? 'bg-[hsl(0_0%_85%)]' : ''}`} onClick={() => { closeMenu(); setActiveTab('signup'); }}><Link to="/signup">S'inscrire</Link></li>
                    </ul>
                </div>
                <button className={` absolute bg-black z-105 h-dvh w-full  ${isMenuOpen? 'opacity-50 transition-opacity duration-300 ease-in-out': 'hidden'}`} onClick={closeMenu}></button>
            </header>
